...
function resolveTargets(target) {
    ...
    if (typeof NodeList !== "undefined" &&
        (target instanceof NodeList || NodeList.prototype.isPrototypeOf(target))) {
        return Array.from(target);
    }
    if (Array.isArray(target)) {
        return target.filter((node) => node && node.nodeType === 1);
    }
    return [];
}
function hasDOM() {
...
export class ReactGlassRenderer {
    constructor(snapshotSelector, snapshotResolution = 1.0) {
        ...
        if (!this.gl)
            throw new Error("reactGlass: WebGL unavailable");
        this.lenses = [];
        this.texture = null;
        this.textureWidth = 0;
    ...
    _initGL() {
      ...
      void main(){
        ...
        const gl = this.gl;
        if (!this.program)
            throw new Error("reactGlass: Shader failed");
    ...
    async captureSnapshot() {
        if (this._capturing || typeof html2canvas === "undefined")
            return;
        ...
        const attemptCapture = async (attempt = 1, maxAttempts = 3, delayMs = 500) => {
            try {
                ...
                const ignoreElementsFunc = (element) => {
                    ...
                    return (element.hasAttribute("data-liquid-ignore") ||
    ...
    _uploadTexture(srcCanvas) {
        ...
        if (!(srcCanvas instanceof HTMLCanvasElement)) {
            ...
            try {
                ...
                console.warn("reactGlass: Unable to convert OffscreenCanvas for upload", e);
                return;
            }
        }
    ...
    removeLens(lens) {
        this.lenses = this.lenses.filter((ln) => ln !== lens);
        if (lens && typeof lens.dispose === "function") {
            lens.dispose();
        }
        if (this.lenses.length === 0 && this._rafId && !this.useExternalTicker) {
    ...
    render() {
        ...
        this.lenses.forEach((ln) => {
            if (ln._mirrorActive && ln._mirrorCtx) {
                ...
                if (mirror.width !== this.canvas.width ||
        ...
        this.lenses.forEach((ln) => {
            if (ln._mirrorActive && ln.rectPx) {
                ...
                const y = Math.max(0, Math.round(this.canvas.height - (top + height) * dpr) - expand);
    ...
    _updateDynamicVideos() {
        ...
        this._videoNodes.forEach((vid) => {
            ...
            if (drawW <= 0 || drawH <= 0)
                return;
            if (this._tmpCanvas.width !== drawW ||
            ...
            try {
                ...
                this._tmpCtx.drawImage(this.staticSnapshotCanvas, texX, texY, texW, texH, 0, 0, drawW, drawH);
    ...
    _updateDynamicNodes() {
        if (this._isScrolling && this._scrollUpdateCounter % 2 !== 0)
            return;
        const gl = this.gl;
        if (!this.texture || !this._dynMeta)
            return;
        const snapRect = this.snapshotTarget.getBoundingClientRect();
        const maxLensZ = this._getMaxLensZ();
        const lensRects = this.lenses.map((ln) => ln.rectPx).filter(Boolean);
        const rectsIntersect = (a, b) => a.left < b.left + b.width &&
        ...
        const compositeVideos = (compositeCtx, dynamicElRect) => {
            this._videoNodes.forEach((vid) => {
                ...
                if (dynamicElRect.left < vidRect.right &&
                    ...
                    compositeCtx.drawImage(vid, xInComposite, yInComposite, wInComposite, hInComposite);
                }
            });
        };
        this._dynamicNodes.forEach((node) => {
            ...
            if (meta.lastCapture) {
                ...
                if (compositeCanvas.width !== drawW ||
                    compositeCanvas.height !== drawH) {
                    compositeCanvas.width = drawW;
                    compositeCanvas.height = drawH;
                }
                this._compositeCtx.clearRect(0, 0, drawW, drawH);
                this._compositeCtx.drawImage(this.staticSnapshotCanvas, texX, texY, rect.width * this.scaleFactor, rect.height * this.scaleFactor, 0, 0, drawW, drawH);
    ...
    addDynamicElement(el) {
        ...
        if (el.closest && el.closest("[data-liquid-ignore]"))
            return;
        if (this._dynamicNodes.some((n) => n.el === el))
            return;
...
export class ReactGlassLens {
    constructor(renderer, element, options) {
        ...
        this.originalBackdropFilter = this.el.style.backdropFilter;
        this.originalWebkitBackdropFilter = this.el.style.webkitBackdropFilter;
        this.originalBackground = this.el.style.background;
        this.originalBackgroundImage = this.el.style.backgroundImage;
    ...
    /* ----------------------------- */
    updateMetrics() {
        const rect = this._mirrorActive && this._baseRect
            ? this._baseRect
    ...
    setShadow(enabled) {
        ...
        const syncShadow = () => {
            if (!this._shadowEl)
                return;
            const r = this._mirrorActive && this._baseRect
                ? this._baseRect
                : this.el.getBoundingClientRect();
    ...
    _bindTiltHandlers() {
        ...
        this._applyTilt = (clientX, clientY) => {
            ...
            const baseTransform = this._savedTransform && this._savedTransform !== "none"
        ...
        this._smoothReset = () => {
            ...
            this.el.style.transformOrigin = `50% 50%`;
            const baseRest = this._savedTransform && this._savedTransform !== "none"
                ? this._savedTransform + " "
                : "";
            let overscrollCompensation = "";
            const bodyStyle = window.getComputedStyle(document.body);
        ...
        this._docPointerMove = (e) => {
            var _a, _b;
            const x = (_a = e.clientX) !== null && _a !== void 0 ? _a : (e.touches && e.touches[0].clientX);
            const y = (_b = e.clientY) !== null && _b !== void 0 ? _b : (e.touches && e.touches[0].clientY);
    ...
    dispose() {
        ...
        this.el.style.pointerEvents = this.originalPointerEvents || "";
        this.el.style.backdropFilter = this.originalBackdropFilter || "";
        this.el.style.webkitBackdropFilter =
            this.originalWebkitBackdropFilter || "";
        this.el.style.background = this.originalBackground || "";
        this.el.style.backgroundImage = this.originalBackgroundImage || "";
 ...
 *  Public API
 * ------------------------------------------------*/
export function reactGlass(userOptions = {}) {
    if (!hasDOM()) {
        console.warn("reactGlass: window/document not available (likely SSR). Skipping init.");
        return;
    }
    ...
    if (typeof cachedNoWebGL === "undefined") {
        ...
        cachedNoWebGL = !testCtx;
    }
    if (cachedNoWebGL) {
        console.warn("reactGlass: WebGL not available – falling back to CSS backdrop-filter.");
        ...
        return fallbackNodes.length === 1 ? fallbackNodes[0] : fallbackNodes;
    }
    if (!rendererSingleton) {
        rendererSingleton = new ReactGlassRenderer(options.snapshot, options.resolution);
    }
    const renderer = rendererSingleton;
    const targets = resolveTargets(options.target);
    if (!targets || targets.length === 0) {
        const targetDesc = typeof options.target === "string" ? options.target : "[custom target]";
        console.warn(`reactGlass: Target element(s) '${targetDesc}' not found.`);
        return;
    }
    const instances = targets.map((el) => renderer.addLens(el, options));
 ...
 * ------------------------------------------------*/
export function registerDynamic(elements) {
    const renderer = rendererSingleton;
    if (!renderer || !renderer.addDynamicElement)
        return;
 ...
 * ------------------------------------------------*/
export function syncWith(config = {}) {
    if (!hasDOM())
        return;
    const renderer = rendererSingleton;
    if (!renderer) {
        console.warn("reactGlass: Please initialize reactGlass *before* calling syncWith().");
        return;
    }
    const G = window.gsap;
    ...
    if (config.locomotiveScroll !== false &&
        LS &&
        !loco &&
...
//# sourceMappingURL=reactGlass.js.map